...
const DAILY_MENU = [
  {
    ...
    price: 28.9,
    image: "https://d2xsxph8kpxj0f.cloudfront.net/310519663658405775/kQVQpftc3b77nEataXztAL/prato-segunda-costela-c23AWFbSHzUb4FYwKaEh8A.webp",
  },
  {
    day: "Terça-Feira",
    ...
    price: 24.9,
    image: "https://d2xsxph8kpxj0f.cloudfront.net/310519663658405775/kQVQpftc3b77nEataXztAL/prato-terca-frango-jhzmuTzgxcSzfcUzVKGgu4.webp",
  },
  {
    day: "Quarta-Feira",
    ...
    price: 26.9,
    image: "https://d2xsxph8kpxj0f.cloudfront.net/310519663658405775/kQVQpftc3b77nEataXztAL/prato-quarta-feijoada-bz6vdkkJF88zeovBHPcg8o.webp",
  },
  {
    day: "Quinta-Feira",
    ...
    price: 32.9,
    image: "https://d2xsxph8kpxj0f.cloudfront.net/310519663658405775/kQVQpftc3b77nEataXztAL/prato-quinta-fileparmegiana-42CCEjvA7M3MJUHbsieZb9.webp",
  },
  {
    day: "Sexta-Feira",
    ...
    price: 34.9,
    image: "https://d2xsxph8kpxj0f.cloudfront.net/310519663658405775/kQVQpftc3b77nEataXztAL/prato-sexta-tilapia-WSpWH9iG7ZmKvtctouUPro.webp",
  },
];
...
export default function Menu() {
  ...
  const categories = useMemo(() => (apiCategories.length > 0 ? apiCategories : getFallbackMenu()), [apiCategories]);
  ...
  return (
    <section id="cardapio" className="bg-[#FAFAF7] py-16">
      <div className="container">
        <h2 className="section-title text-2xl font-bold text-[#2C1810] mb-3" style={{ fontFamily: "var(--font-display)" }}>Cardápio</h2>
        <p className="text-sm text-[#7A6555] mb-10">
          {usingFallback ? "Cardápio temporário (Painel Offline)." : "Pratos sincronizados automaticamente com o painel administrativo."}
        </p>
        {isLoadingMenu ? (
          <div className="flex items-center justify-center py-12 text-[#E8521A]">
            <Loader2 className="w-6 h-6 animate-spin mr-2" /><span className="text-sm font-semibold">Carregando cardápio...</span>
          </div>
        ) : (
          <div className="space-y-12">
            {categories.map((category) => (
              <div key={category.id}>
                <h3 className="section-title text-xl font-bold text-[#2C1810] mb-6" style={{ fontFamily: "var(--font-display)" }}>{category.name}</h3>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {category.items.map((item) => (
                    <div key={item.key} className="product-card bg-white rounded-2xl overflow-hidden shadow-sm border border-[#EDE8E2] fade-in-up">
                      {item.imageUrl ? (
                        <img src={item.imageUrl} alt={item.name} className="w-full h-64 object-cover" loading="lazy" />
                      ) : (
                        <div className="w-full h-40 bg-[#F0EDE8] flex items-center justify-center text-[#B0A090] text-sm font-semibold">Imagem não cadastrada</div>
                      )}
                      <div className="p-5">
                        <p className="text-xs font-bold text-[#E8521A] uppercase tracking-widest mb-1">{item.label || item.categoryName}</p>
                        <h4 className="text-lg font-bold text-[#2C1810] mb-1" style={{ fontFamily: "var(--font-display)" }}>{item.name}</h4>
                        {item.description && <p className="text-sm text-[#7A6555] mb-4 leading-relaxed">{item.description}</p>}
                        <div className="flex items-center justify-between">
                          <span className="text-2xl font-bold text-[#E8521A]" style={{ fontFamily: "var(--font-display)" }}>R$ {item.price.toFixed(2).replace(".", ",")}</span>
                          <button onClick={() => handleAdd(item)} className="flex items-center gap-2 bg-[#E8521A] hover:bg-[#C94415] text-white text-sm font-semibold px-4 py-2.5 rounded-xl transition-all">
